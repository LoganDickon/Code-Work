    ...
    switch(breath.state)
    {
        ...
        case BreathState::OUT:
          ...
          if(move.state == MoveState::IDLE)
          {
            ...
            if (volComp == NULL)
                volumeCompensationFactor = 1.0F;
            else
                volumeCompensationFactor = (1.0F + volComp->val);

            int bpMin, bpMax, bpSamples;
            vg->bpRange(&bpMin, &bpMax, &bpSamples);

            //
            // Compensate for low compliance lungs which is signaled by elevated peek pressure readings
            //

// First pass R&C adjustment            
//            int bpGate = 12;
//            float bpComp = .005;
            

            int bpGate = vol_set / 20;
            float bpComp = .007;
            
            if (bpMax > bpGate)
            {
                volumeCompensationFactor *= (1.0F + (bpComp * (bpMax - bpGate)));
                Serial.printf("bpMax %d bpGate %d bpComp %f vComp %2.4f \n", bpMax, bpGate, bpComp, volumeCompensationFactor);
            }
